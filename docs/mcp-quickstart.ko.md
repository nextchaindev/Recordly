...
### 가장 쉬운 방법: Claude 데스크톱 앱에 확장 프로그램으로 설치

같이 받은 `Recordly-MCP.mcpb`를 더블클릭하고 **설치**를 누릅니다. 안 열리면 Claude 앱 → 설정 →
확장 프로그램 → 고급 설정 → 확장 프로그램 설치… 에서 파일을 고릅니다. Node.js, JSON 편집, 명령어가
필요 없고, 새 채팅에서 바로 `recordly_*` 도구가 보입니다. (확장 프로그램은 Claude 앱 채팅용입니다.
Code 탭이나 Cursor는 아래 파일 복사 방법을 씁니다.)

### 명령어 없이 파일로 등록하기 (Code 탭 / Cursor)
